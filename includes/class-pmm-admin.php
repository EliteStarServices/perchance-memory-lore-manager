...
class PMM_Admin {
	...
	public function render_page() {
		...
		$entries_cleared = isset($_GET['pmm_entries_cleared']) ? (int) $_GET['pmm_entries_cleared'] : 0;
		$versions_deleted = isset($_GET['pmm_versions_deleted']) ? (int) $_GET['pmm_versions_deleted'] : 0;
		$entity_updated = isset($_GET['pmm_entity_updated']) ? (int) $_GET['pmm_entity_updated'] : 0;
		$entity_entries_updated = isset($_GET['pmm_entity_entries_updated']) ? (int) $_GET['pmm_entity_entries_updated'] : 0;
		...
		$global_entity_names_saved = isset($_GET['pmm_global_entity_names_saved']) ? (int) $_GET['pmm_global_entity_names_saved'] : -1;
		...
		$db_rebuild_job      = isset($_GET['pmm_db_rebuild_job'])      ? sanitize_key((string) wp_unslash($_GET['pmm_db_rebuild_job']))  : '';
		$db_rebuild_total    = isset($_GET['pmm_db_rebuild_total'])    ? (int) $_GET['pmm_db_rebuild_total']    : 0;
		$db_rebuild_offset   = isset($_GET['pmm_db_rebuild_offset'])   ? (int) $_GET['pmm_db_rebuild_offset']   : 0;
		$db_rebuild_percent  = isset($_GET['pmm_db_rebuild_percent'])  ? max(0, min(99, (int) $_GET['pmm_db_rebuild_percent'])) : 0;
		...
		$db_bulk_restored = isset($_GET['pmm_db_bulk_restored']) ? (int) $_GET['pmm_db_bulk_restored'] : -1;
		...
		$prune_global_mode = isset($_GET['pmm_prune_global_mode']) ? (int) $_GET['pmm_prune_global_mode'] : 0;
		...
		$prune_global_dup = isset($_GET['pmm_prune_global_dup']) ? (int) $_GET['pmm_prune_global_dup'] : 0;
		...
		$prune_entity_cap_removed = isset($_GET['pmm_prune_entity_cap_removed']) ? (int) $_GET['pmm_prune_entity_cap_removed'] : 0;
		...
		$global_entity_names = $this->get_global_entity_names_option();
		$global_entity_names_text = implode("\n", $global_entity_names);
		$global_entity_match_counts = [];
		$db_stats = PMM_DB::stats();
		$db_unknown_rows = PMM_DB::get_unknown_entries(100);
		$db_perchance_export_entries = [];
		$db_perchance_export_text = '';
		if ((int) ($db_stats['total_entries'] ?? 0) > 0) {
			$db_perchance_export_entries = PMM_DB::get_non_pruned_entries_chronological();
			if (!empty($db_perchance_export_entries)) {
				$db_perchance_export_text = implode("\n\n", $db_perchance_export_entries);
			}
		}
		$db_browse_status      = isset($_GET['pmm_db_browse_status'])  ? sanitize_key((string) wp_unslash($_GET['pmm_db_browse_status']))  : 'known';
		if (!in_array($db_browse_status, ['all', 'known', 'unknown', 'pruned'], true)) { $db_browse_status = 'known'; }
		$db_browse_mode        = isset($_GET['pmm_db_browse_mode']) ? sanitize_key((string) wp_unslash($_GET['pmm_db_browse_mode'])) : 'standard';
		if (!in_array($db_browse_mode, ['standard', 'entity_exact_only', 'entity_contains', 'keywords'], true)) {
			$db_browse_mode = 'standard';
		}
		$db_browse_search      = isset($_GET['pmm_db_browse_search'])  ? sanitize_text_field((string) wp_unslash($_GET['pmm_db_browse_search'])) : '';
		$db_browse_entity      = isset($_GET['pmm_db_browse_entity'])  ? sanitize_text_field((string) wp_unslash($_GET['pmm_db_browse_entity'])) : '';
		$db_browse_keywords    = isset($_GET['pmm_db_browse_keywords']) ? sanitize_text_field((string) wp_unslash($_GET['pmm_db_browse_keywords'])) : '';
		$db_browse_from        = isset($_GET['pmm_db_browse_from']) ? max(0, (int) $_GET['pmm_db_browse_from']) : 0;
		$db_browse_to          = isset($_GET['pmm_db_browse_to']) ? max(0, (int) $_GET['pmm_db_browse_to']) : 0;
		$db_browse_skip_legacy = isset($_GET['pmm_db_browse_skip']) ? max(0, (int) $_GET['pmm_db_browse_skip']) : 0;
		if ($db_browse_from <= 0 && $db_browse_skip_legacy > 0) {
			$db_browse_from = $db_browse_skip_legacy + 1;
		}
		if ($db_browse_from === 0) {
			$db_browse_to = 0;
		}
		if ($db_browse_from > 0 && $db_browse_to > 0 && $db_browse_to < $db_browse_from) {
			$db_browse_to = $db_browse_from;
		}
		$db_browse_open_ended  = $db_browse_from > 0 && $db_browse_to === 0;
		$db_browse_range_enabled = $db_browse_from > 0 && ($db_browse_to === 0 || $db_browse_to >= $db_browse_from);
		$db_browse_skip        = $db_browse_range_enabled ? max(0, $db_browse_from - 1) : 0;
		$db_browse_window_size = (!$db_browse_open_ended && $db_browse_range_enabled) ? max(1, ($db_browse_to - $db_browse_from + 1)) : 0;
		$db_browse_page        = isset($_GET['pmm_db_browse_page'])    ? max(1, (int) $_GET['pmm_db_browse_page'])  : 1;
		$db_browse_per_page    = (!$db_browse_open_ended && $db_browse_range_enabled) ? min(500, $db_browse_window_size) : 50;
		if (!$db_browse_open_ended && $db_browse_range_enabled) {
			$db_browse_page = 1;
		}
		if ($db_browse_range_enabled) {
			$db_browse_offset = $db_browse_skip + (($db_browse_page - 1) * $db_browse_per_page);
		} else {
			$db_browse_offset = ($db_browse_page - 1) * $db_browse_per_page;
		}
		$db_browse_total       = 0;
		$db_browse_total_found = 0;
		$db_browse_rows        = [];
		$db_query_preview_cap = 2000;
		$db_query_preview_rows = [];
		$db_query_preview_text = '';
		$db_query_preview_truncated = false;
		$db_all_entity_names   = [];
		if ((int) ($db_stats['total_entries'] ?? 0) > 0) {
			$db_all_entity_names = PMM_DB::get_all_entity_names();
			$db_browse_total_found = PMM_DB::count_entries_filtered($db_browse_status, $db_browse_search, $db_browse_entity, $db_browse_mode, $db_browse_keywords);
			if (!$db_browse_open_ended && $db_browse_range_enabled) {
				$db_browse_total = max(0, min($db_browse_window_size, $db_browse_total_found - $db_browse_skip));
			} elseif ($db_browse_open_ended) {
				$db_browse_total = max(0, $db_browse_total_found - $db_browse_skip);
			} else {
				$db_browse_total = max(0, $db_browse_total_found);
			}
			if ($db_browse_offset < $db_browse_total_found) {
				if (!$db_browse_open_ended && $db_browse_range_enabled) {
					$db_browse_rows = PMM_DB::get_entries_filtered_preview($db_browse_status, $db_browse_search, $db_browse_entity, $db_browse_mode, $db_browse_keywords, $db_browse_skip, $db_browse_window_size);
					$db_browse_per_page = max(1, count($db_browse_rows));
				} else {
					$db_browse_rows = PMM_DB::get_entries_with_entity_names($db_browse_per_page, $db_browse_offset, $db_browse_status, $db_browse_search, $db_browse_entity, $db_browse_mode, $db_browse_keywords);
				}
			}
			if ($db_browse_entity !== '' && $db_browse_total > 0) {
				$db_query_preview_limit = (!$db_browse_open_ended && $db_browse_range_enabled) ? $db_browse_window_size : $db_query_preview_cap;
				$db_query_preview_rows = PMM_DB::get_entries_filtered_preview($db_browse_status, $db_browse_search, $db_browse_entity, $db_browse_mode, $db_browse_keywords, $db_browse_skip, $db_query_preview_limit);
				$db_query_preview_truncated = count($db_query_preview_rows) < $db_browse_total;
				$preview_lines = [];
				foreach ($db_query_preview_rows as $preview_row) {
					$preview_lines[] = sprintf(
						'[%1$d] %2$s | %3$s | %4$s\n%5$s',
						(int) ($preview_row['id'] ?? 0),
						trim((string) ($preview_row['section_name'] ?? '')),
						trim((string) ($preview_row['status'] ?? '')),
						trim((string) ($preview_row['entity_names'] ?? '')),
						trim((string) ($preview_row['entry_text'] ?? ''))
					);
				}
				$db_query_preview_text = implode("\n\n", $preview_lines);
			}
		}
		$db_dedupe_preview = get_transient('pmm_db_dedupe_preview_' . get_current_user_id());
		if (!is_array($db_dedupe_preview)) {
			$db_dedupe_preview = [];
		}
		$db_dedupe_candidates = isset($db_dedupe_preview['candidates']) && is_array($db_dedupe_preview['candidates']) ? $db_dedupe_preview['candidates'] : [];
		$db_dedupe_scope = isset($db_dedupe_preview['scope']) ? sanitize_key((string) $db_dedupe_preview['scope']) : 'all';
		if (!in_array($db_dedupe_scope, ['all', 'unknown', 'known'], true)) {
			$db_dedupe_scope = 'all';
		}
		$db_dedupe_threshold = isset($db_dedupe_preview['threshold']) ? max(0.70, min(0.99, (float) $db_dedupe_preview['threshold'])) : 0.92;
		$db_dedupe_limit = isset($db_dedupe_preview['limit']) ? max(25, min(1000, (int) $db_dedupe_preview['limit'])) : 300;
		...
		if (!empty($data['cleaned_data']) && is_array($data['cleaned_data'])) {
			...
			if (!empty($global_entity_names)) {
				$global_entity_match_counts = $this->count_global_entity_matches((array) $data['cleaned_data'], $global_entity_names);
			}
		}
		...
		<div class="wrap pmm-wrap">
			...
			<?php if ($entries_cleared) : ?>
				<div class="notice notice-success"><p><?php echo esc_html($versions_deleted ? __('Cleared working entries, review queues, and saved version files.', 'perchance-memory-manager') : __('Cleared working entries and review queues. Saved version files were kept.', 'perchance-memory-manager')); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($entity_entries_updated > 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html(sprintf(__('Entity Review updated entries for %d entities directly in latest output.', 'perchance-memory-manager'), $entity_entries_updated)); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($global_entity_names_saved >= 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html(sprintf(__('Saved %d global entity names. Prune/unreferenced review now uses this list when present.', 'perchance-memory-manager'), max(0, $global_entity_names_saved))); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($db_rebuilding && $db_rebuild_job !== '') : ?>
				<div class="notice notice-info" id="pmm-db-rebuild-notice">
					...
					<?php echo esc_html(sprintf(__('%1$d / %2$d entries inserted (%3$d%%).', 'perchance-memory-manager'), min($db_rebuild_offset, $db_rebuild_total), $db_rebuild_total, $db_rebuild_percent)); ?></p>
					<progress max="100" value="<?php echo esc_attr((string) $db_rebuild_percent); ?>" style="width:100%;height:14px;"></progress>
					<form id="pmm-db-rebuild-continue-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top:8px;">
						<?php wp_nonce_field('pmm_db_rebuild_continue'); ?>
						<input type="hidden" name="action" value="pmm_db_rebuild_continue">
						<input type="hidden" name="pmm_db_rebuild_job" value="<?php echo esc_attr($db_rebuild_job); ?>">
						<noscript><?php submit_button(__('Continue Next Batch', 'perchance-memory-manager'), 'secondary', 'submit', false); ?></noscript>
					</form>
				</div>
				<script>
				document.addEventListener('DOMContentLoaded', function () {
					var form = document.getElementById('pmm-db-rebuild-continue-form');
					if (form) { setTimeout(function () { form.submit(); }, 300); }
				});
				</script>
			<?php endif; ?>

			<?php if ($db_tables_cleared) : ?>
				<div class="notice notice-success"><p><?php esc_html_e('DB tables cleared.', 'perchance-memory-manager'); ?></p></div>
			<?php endif; ?>

			<?php if ($db_rescanned >= 0) : ?>
				<div class="notice notice-info"><p><?php echo esc_html(sprintf(__('Re-scan complete: %d unknown entries were moved to known entities.', 'perchance-memory-manager'), max(0, $db_rescanned))); ?></p></div>
			<?php endif; ?>

			<?php if ($db_retagged >= 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html($db_retagged ? __('Unknown entry was re-tagged and removed from Unknown.', 'perchance-memory-manager') : __('Could not re-tag entry. Select at least one existing entity or add a new entity name.', 'perchance-memory-manager')); ?></p></div>
			<?php endif; ?>

			<?php if ($db_entry_updated >= 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html($db_entry_updated ? __('Unknown entry text was updated.', 'perchance-memory-manager') : __('Could not update unknown entry text.', 'perchance-memory-manager')); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($db_bulk_restored >= 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html(sprintf(_n('%d pruned entry restored.', '%d pruned entries restored.', max(0, $db_bulk_restored), 'perchance-memory-manager'), max(0, $db_bulk_restored))); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($db_dedupe_previewed >= 0) : ?>
				<div class="notice notice-info"><p><?php echo esc_html(sprintf(__('DB dedupe preview generated: %d candidate pairs ready for review.', 'perchance-memory-manager'), max(0, $db_dedupe_previewed))); ?></p></div>
			<?php endif; ?>

			<?php if ($db_dedupe_applied >= 0) : ?>
				<div class="notice notice-success"><p><?php echo esc_html(sprintf(__('DB dedupe review applied: %1$d pairs reviewed, %2$d entries marked as pruned.', 'perchance-memory-manager'), max(0, $db_dedupe_reviewed), max(0, $db_dedupe_applied))); ?></p></div>
			<?php endif; ?>
			...
			<?php if ($entity_pruned) : ?>
				<div class="notice notice-success"><p><?php echo esc_html(sprintf(__('Entity prune completed: %1$d -> %2$d entries (exact duplicates removed: %3$d, near duplicates removed: %4$d, global duplicates removed: %5$d, stale removed: %6$d, unreferenced removed: %7$d, missing selected entity name removed: %8$d, global entity-name cap removed: %9$d, critical entries preserved: %10$d, trimmed to cap: %11$d).', 'perchance-memory-manager'), max(0, $prune_before), max(0, $prune_after), max(0, $prune_exact), max(0, $prune_near), max(0, $prune_global_dup), max(0, $prune_stale), max(0, $prune_unref), max(0, $prune_entity_mismatch), max(0, $prune_entity_cap_removed), max(0, $prune_critical), max(0, $prune_trimmed))); ?></p></div>
			<?php endif; ?>
			...
			<?php if (false && $rules_dirty) : ?>
				<div class="notice notice-warning">
					...
					<?php if ($has_last_output) : ?>
						<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:8px 0 0 0;">
							<?php wp_nonce_field('pmm_reprocess_last_output'); ?>
							<input type="hidden" name="action" value="pmm_reprocess_last_output">
							<?php submit_button(__('Reprocess Last Output Now', 'perchance-memory-manager'), 'secondary', 'submit', false); ?>
						</form>
			...
			<div class="pmm-card">
				<details class="pmm-collapsible-section pmm-collapsible-root">
					<summary><strong><?php esc_html_e('Upload and Process', 'perchance-memory-manager'); ?></strong></summary>
				<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:0 0 12px 0;padding:10px;border:1px solid #dcdcde;background:#fff;">
					<?php wp_nonce_field('pmm_clear_entries_store'); ?>
					<input type="hidden" name="action" value="pmm_clear_entries_store">
					<p style="margin:0 0 8px 0;"><strong><?php esc_html_e('Reset Working Entries', 'perchance-memory-manager'); ?></strong></p>
					<p class="description" style="margin:0 0 8px 0;"><?php esc_html_e('Clears active entries/output and review queues so you can repopulate cleanly during testing.', 'perchance-memory-manager'); ?></p>
					<label style="display:block;margin:0 0 8px 0;"><input type="checkbox" name="pmm_clear_saved_version_files" value="1"> <?php esc_html_e('Also delete saved version files/history', 'perchance-memory-manager'); ?></label>
					<?php submit_button(__('Clear Working Entries', 'perchance-memory-manager'), 'delete', 'submit', false, ['onclick' => "return confirm('Clear working entries and review queues now?');"]); ?>
				</form>
			...
			<?php if ((int) ($db_stats['total_entries'] ?? 0) > 0) : ?>
				<div class="pmm-card">
					<details class="pmm-collapsible-section pmm-collapsible-root">
						...
						<p class="description"><?php esc_html_e('Chronological prune scan in 50-entry blocks. Choose a mode: tagged exact entity, entries containing an entity name, or keyword query. Use Skip to jump past earlier matches.', 'perchance-memory-manager'); ?></p>
						<form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>" style="margin:8px 0;display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;">
							...
							<label><?php esc_html_e('Scan mode', 'perchance-memory-manager'); ?><br>
								<select name="pmm_db_browse_mode">
									<option value="standard" <?php selected($db_browse_mode, 'standard'); ?>><?php esc_html_e('Standard browser', 'perchance-memory-manager'); ?></option>
									<option value="entity_exact_only" <?php selected($db_browse_mode, 'entity_exact_only'); ?>><?php esc_html_e('Only selected tagged entity', 'perchance-memory-manager'); ?></option>
									<option value="entity_contains" <?php selected($db_browse_mode, 'entity_contains'); ?>><?php esc_html_e('Entry contains selected entity name', 'perchance-memory-manager'); ?></option>
									<option value="keywords" <?php selected($db_browse_mode, 'keywords'); ?>><?php esc_html_e('Keyword scan', 'perchance-memory-manager'); ?></option>
								</select>
							</label>
							<label><?php esc_html_e('Status', 'perchance-memory-manager'); ?><br>
								<select name="pmm_db_browse_status">
									<?php foreach (['known' => 'Known', 'unknown' => 'Unknown', 'pruned' => 'Pruned', 'all' => 'All'] as $sv => $sl) : ?>
										<option value="<?php echo esc_attr($sv); ?>" <?php selected($db_browse_status, $sv); ?>><?php echo esc_html($sl); ?></option>
									<?php endforeach; ?>
								</select>
							</label>
							<?php if (!empty($db_all_entity_names)) : ?>
							<label><?php esc_html_e('Entity', 'perchance-memory-manager'); ?><br>
								<select name="pmm_db_browse_entity">
									<option value=""><?php esc_html_e('— Any —', 'perchance-memory-manager'); ?></option>
									<?php foreach ($db_all_entity_names as $en) : ?>
										<option value="<?php echo esc_attr((string) $en); ?>" <?php selected($db_browse_entity, $en); ?>><?php echo esc_html((string) $en); ?></option>
									<?php endforeach; ?>
								</select>
							</label>
							<?php endif; ?>
							<label><?php esc_html_e('Search text (standard mode)', 'perchance-memory-manager'); ?><br>
								<input type="search" name="pmm_db_browse_search" value="<?php echo esc_attr($db_browse_search); ?>" class="regular-text" placeholder="<?php esc_attr_e('keyword…', 'perchance-memory-manager'); ?>">
							</label>
							<label><?php esc_html_e('Keywords (keyword mode)', 'perchance-memory-manager'); ?><br>
								<input type="search" name="pmm_db_browse_keywords" value="<?php echo esc_attr($db_browse_keywords); ?>" class="regular-text" placeholder="<?php esc_attr_e('alpha beta', 'perchance-memory-manager'); ?>">
							</label>
							<label><?php esc_html_e('Show matches from', 'perchance-memory-manager'); ?><br>
								<input type="number" min="0" step="1" name="pmm_db_browse_from" value="<?php echo esc_attr((string) $db_browse_from); ?>" style="width:110px;">
							</label>
							<label><?php esc_html_e('through', 'perchance-memory-manager'); ?><br>
								<input type="number" min="0" step="1" name="pmm_db_browse_to" value="<?php echo esc_attr((string) $db_browse_to); ?>" style="width:110px;">
							</label>
							<input type="hidden" name="pmm_db_browse_page" value="1">
							<?php submit_button(__('Search', 'perchance-memory-manager'), 'secondary', 'submit', false, ['style' => 'margin:0;']); ?>
						</form>
						<p class="description" style="margin-top:0;"><?php esc_html_e('Set "Show matches from" to 0 (default) to disable range mode. When enabled, paging is bypassed and only that match window is shown.', 'perchance-memory-manager'); ?></p>
						<?php if ($db_browse_entity !== '' && $db_browse_total > 0) : ?>
							<div style="margin:8px 0;padding:10px;border:1px solid #dcdcde;background:#fff;">
								<p class="description" style="margin-top:0;"><?php echo esc_html(sprintf(__('Full query preview for entity "%1$s" (%2$d match(es) in current window). Use this area to act on the whole matched window without paging.', 'perchance-memory-manager'), $db_browse_entity, max(0, (int) $db_browse_total))); ?></p>
								<textarea class="large-text code" rows="10" readonly style="white-space:pre;overflow:auto;"><?php echo esc_textarea($db_query_preview_text); ?></textarea>
								<?php if ($db_query_preview_truncated) : ?>
									<p class="description" style="margin:6px 0 0 0;"><?php echo esc_html(sprintf(__('Textarea preview shows the first %d matches for readability. Query-level actions below still target all current matches.', 'perchance-memory-manager'), max(1, (int) $db_query_preview_cap))); ?></p>
								<?php endif; ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
									<?php wp_nonce_field('pmm_db_apply_query_action'); ?>
									<input type="hidden" name="action" value="pmm_db_apply_query_action">
									<input type="hidden" name="pmm_db_browse_status" value="<?php echo esc_attr($db_browse_status); ?>">
									<input type="hidden" name="pmm_db_browse_mode" value="<?php echo esc_attr($db_browse_mode); ?>">
									...
									<input type="hidden" name="pmm_db_browse_keywords" value="<?php echo esc_attr($db_browse_keywords); ?>">
									<input type="hidden" name="pmm_db_browse_from" value="<?php echo esc_attr((string) $db_browse_from); ?>">
									<input type="hidden" name="pmm_db_browse_to" value="<?php echo esc_attr((string) $db_browse_to); ?>">
									<input type="hidden" name="pmm_db_browse_page" value="<?php echo esc_attr((string) $db_browse_page); ?>">
									<?php if ($db_browse_status === 'pruned') : ?>
										<button type="submit" class="button button-primary" name="pmm_db_query_action_type" value="restore" onclick="return confirm('<?php echo esc_js(__('Restore all matched pruned entries from this full query?', 'perchance-memory-manager')); ?>');"><?php esc_html_e('Restore All Matches', 'perchance-memory-manager'); ?></button>
									<?php elseif ($db_browse_status === 'all') : ?>
										<button type="submit" class="button button-primary" name="pmm_db_query_action_type" value="prune" onclick="return confirm('<?php echo esc_js(__('Prune all matched entries from this full query?', 'perchance-memory-manager')); ?>');"><?php esc_html_e('Prune All Matches', 'perchance-memory-manager'); ?></button>
										<button type="submit" class="button" name="pmm_db_query_action_type" value="restore" onclick="return confirm('<?php echo esc_js(__('Restore all matched pruned entries from this full query?', 'perchance-memory-manager')); ?>');"><?php esc_html_e('Restore All Matches', 'perchance-memory-manager'); ?></button>
									<?php else : ?>
										<button type="submit" class="button button-primary" name="pmm_db_query_action_type" value="prune" onclick="return confirm('<?php echo esc_js(__('Prune all matched entries from this full query?', 'perchance-memory-manager')); ?>');"><?php esc_html_e('Prune All Matches', 'perchance-memory-manager'); ?></button>
									<?php endif; ?>
								</form>
						...
						<?php endif; ?>
						<?php if ($db_browse_total > 0) : ?>
							<?php
							$db_browse_total_pages = max(1, (int) ceil($db_browse_total / $db_browse_per_page));
							$db_browse_page = min($db_browse_page, $db_browse_total_pages);
							?>
							<?php if ($db_browse_range_enabled && !$db_browse_open_ended) : ?>
								<p class="description"><?php echo esc_html(sprintf(__('Showing requested match window %1$d through %2$d. Loaded %3$d match(es). Total matches for current filters: %4$d.', 'perchance-memory-manager'), max(1, (int) $db_browse_from), max((int) $db_browse_from, (int) $db_browse_to), count($db_browse_rows), max(0, (int) $db_browse_total_found))); ?></p>
							<?php elseif ($db_browse_open_ended) : ?>
								<p class="description"><?php echo esc_html(sprintf(__('Showing matches from %1$d onward in 50-entry blocks. Remaining matches: %2$d — block %3$d of %4$d.', 'perchance-memory-manager'), max(1, (int) $db_browse_from), max(0, (int) $db_browse_total), $db_browse_page, $db_browse_total_pages)); ?></p>
							<?php else : ?>
								<p class="description"><?php echo esc_html(sprintf(__('Showing 50-entry review blocks. Total matches: %1$d — block %2$d of %3$d.', 'perchance-memory-manager'), $db_browse_total, $db_browse_page, $db_browse_total_pages)); ?></p>
							<?php endif; ?>
							<?php if ((!$db_browse_range_enabled || $db_browse_open_ended) && $db_browse_total_pages > 1) : ?>
							<p style="margin:6px 0;">
								<?php if ($db_browse_page > 1) : ?>
									<a class="button" href="<?php echo esc_url(add_query_arg(['pmm_db_browse_page' => $db_browse_page - 1, 'pmm_db_browse_status' => $db_browse_status, 'pmm_db_browse_mode' => $db_browse_mode, 'pmm_db_browse_entity' => $db_browse_entity, 'pmm_db_browse_search' => $db_browse_search, 'pmm_db_browse_keywords' => $db_browse_keywords, 'pmm_db_browse_from' => $db_browse_from, 'pmm_db_browse_to' => $db_browse_to])); ?>"><?php esc_html_e('← Prev', 'perchance-memory-manager'); ?></a>
								<?php endif; ?>
								<?php if ($db_browse_page < $db_browse_total_pages) : ?>
									<a class="button" href="<?php echo esc_url(add_query_arg(['pmm_db_browse_page' => $db_browse_page + 1, 'pmm_db_browse_status' => $db_browse_status, 'pmm_db_browse_mode' => $db_browse_mode, 'pmm_db_browse_entity' => $db_browse_entity, 'pmm_db_browse_search' => $db_browse_search, 'pmm_db_browse_keywords' => $db_browse_keywords, 'pmm_db_browse_from' => $db_browse_from, 'pmm_db_browse_to' => $db_browse_to])); ?>"><?php esc_html_e('Next →', 'perchance-memory-manager'); ?></a>
								<?php endif; ?>
							</p>
							<?php endif; ?>
							<div style="overflow:auto;max-height:480px;border:1px solid #dcdcde;background:#fff;">
								...
								<?php foreach ($db_browse_rows as $br) :
									$br_id     = (int) ($br['id'] ?? 0);
									$br_status = (string) ($br['status'] ?? '');
									$br_color  = $br_status === 'known' ? '#2e7d32' : ($br_status === 'unknown' ? '#e65100' : '#9e9e9e');
									$br_save_label = $br_status === 'pruned' ? __('Save + Restore', 'perchance-memory-manager') : __('Save', 'perchance-memory-manager');
								?>
									<tr>
										...
										<td style="vertical-align:top;white-space:pre-wrap;"><?php echo esc_html((string) ($br['entry_text'] ?? '')); ?></td>
										<td style="vertical-align:top;">
											<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:0;">
												...
												<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
													...
													<?php submit_button($br_save_label, 'secondary', 'submit', false, ['style' => 'margin:0;']); ?>
													<button type="submit" class="button button-link-delete" style="margin:0;" onclick="document.getElementById('pmm_br_action_<?php echo esc_js((string) $br_id); ?>').value='prune';return true;"><?php esc_html_e('Delete', 'perchance-memory-manager'); ?></button>
												</div>
											</form>
										</td>
									</tr>
							...
							<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="pmm_db_browser_bulk_update_form" style="margin-top:6px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
								...
								<input type="hidden" name="pmm_db_browse_to" value="<?php echo esc_attr((string) $db_browse_to); ?>">
								<input type="hidden" name="pmm_db_browse_page" value="<?php echo esc_attr((string) $db_browse_page); ?>">
								<?php submit_button($db_browse_status === 'pruned' ? __('Save + Restore Selected', 'perchance-memory-manager') : __('Save Selected Edits', 'perchance-memory-manager'), 'secondary', 'submit', false); ?>
							</form>
							<script>
							document.addEventListener('DOMContentLoaded', function () {
						...
						<?php else : ?>
							<p class="description"><?php esc_html_e('Select a filter above and click Search to browse entries.', 'perchance-memory-manager'); ?></p>
						<?php endif; ?>
					</details>
				</div>
			<?php endif; ?>
			...
			<?php
			// Entity Workspace: show if there is a loaded output OR if the DB has entities to populate from
			$has_entity_workspace = (!empty($data) && !empty($data['cleaned_data']) && is_array($data['cleaned_data']))
				|| !empty($db_all_entity_names);
			?>
			<?php if ($has_entity_workspace) : ?>
				<div class="pmm-card">
					<details class="pmm-collapsible-section pmm-collapsible-root">
					...
					<?php if (empty($data['cleaned_data'])) : ?>
						<p class="description" style="color:#b32d2e;"><strong><?php esc_html_e('Note:', 'perchance-memory-manager'); ?></strong> <?php esc_html_e('No processed output is loaded. To edit entity buckets you need to upload and process a file first, or use the DB Entry Browser above.', 'perchance-memory-manager'); ?></p>
					<?php endif; ?>
					...
					<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
						...
						<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="pmm-prune-entity-form">
							...
							<p>
								<label><?php esc_html_e('Max entries to keep', 'perchance-memory-manager'); ?>
									<input type="number" name="pmm_prune_max_keep" min="50" max="500" step="1" value="350" style="width:110px;">
								</label>
							...
							<p>
								<label><input type="checkbox" name="pmm_prune_global_mode" value="1" <?php checked($prune_global_mode, 1); ?>> <?php esc_html_e('Global mode: evaluate all sections/entities and collect one combined review queue before applying', 'perchance-memory-manager'); ?></label>
								<span class="description" style="margin-left:6px;"><?php esc_html_e('World Building is excluded from global prune and is only pruned when selected directly.', 'perchance-memory-manager'); ?></span>
							</p>
							<p>
								<label><input type="checkbox" name="pmm_prune_global_duplicate_detection" value="1"> <?php esc_html_e('Global duplicate detection: remove duplicates across all buckets during this run (opt-in; can be slow on large files)', 'perchance-memory-manager'); ?></label>
							</p>
							<p>
								<label><input type="checkbox" name="pmm_prune_global_entity_name_cap_enabled" value="1" id="pmm_prune_global_entity_name_cap_enabled"> <?php esc_html_e('Global entity-name cap (opt-in)', 'perchance-memory-manager'); ?></label>
								<label style="margin-left:8px;"><?php esc_html_e('Max counted entries', 'perchance-memory-manager'); ?>
									<input type="number" name="pmm_prune_global_entity_name_cap" id="pmm_prune_global_entity_name_cap" min="1" max="4000" step="1" value="1350" style="width:110px;" disabled>
								</label>
								<span class="description" style="margin-left:6px;"><?php esc_html_e('When enabled, each entry counts at most once toward this cap in global mode.', 'perchance-memory-manager'); ?></span>
							</p>
							<p>
									<label><input type="checkbox" name="pmm_prune_global_entity_name_cap_ignore_enabled" value="1" id="pmm_prune_global_entity_name_cap_ignore_enabled" checked> <?php esc_html_e('Ignore one entity during global prune (including global entity-name cap)', 'perchance-memory-manager'); ?></label>
								<label style="margin-left:8px;"><?php esc_html_e('Ignored entity', 'perchance-memory-manager'); ?>
									<input type="text" name="pmm_prune_global_entity_name_cap_ignore_entity" id="pmm_prune_global_entity_name_cap_ignore_entity" value="DJa" style="width:140px;" disabled>
								</label>
							</p>
							<p>
							...
							<p>
								<label><input type="checkbox" name="pmm_prune_require_entity_name_match" value="1" id="pmm_prune_require_entity_name_match"> <?php esc_html_e('Entity-name check: remove entries that do not mention the selected entity name (only in single-entity mode)', 'perchance-memory-manager'); ?></label>
							</p>
							<script>
							(function () {
								var globalModeCheckbox = document.querySelector('input[name="pmm_prune_global_mode"]');
								var entityNameCheckbox = document.getElementById('pmm_prune_require_entity_name_match');
								var globalEntityCapEnabled = document.getElementById('pmm_prune_global_entity_name_cap_enabled');
								var globalEntityCapInput = document.getElementById('pmm_prune_global_entity_name_cap');
								var globalEntityCapIgnoreEnabled = document.getElementById('pmm_prune_global_entity_name_cap_ignore_enabled');
								var globalEntityCapIgnoreEntity = document.getElementById('pmm_prune_global_entity_name_cap_ignore_entity');
								var updateEntityNameDisabled = function () {
									if (globalModeCheckbox && entityNameCheckbox) {
										entityNameCheckbox.disabled = globalModeCheckbox.checked;
										if (globalModeCheckbox.checked) {
											entityNameCheckbox.checked = false;
										}
									}
									if (globalEntityCapEnabled && globalEntityCapInput) {
										var capEnabled = globalModeCheckbox ? (globalModeCheckbox.checked && globalEntityCapEnabled.checked) : globalEntityCapEnabled.checked;
										globalEntityCapInput.disabled = !capEnabled;
										if (globalEntityCapIgnoreEnabled && globalEntityCapIgnoreEntity) {
											globalEntityCapIgnoreEnabled.disabled = !capEnabled;
											globalEntityCapIgnoreEntity.disabled = !capEnabled || !globalEntityCapIgnoreEnabled.checked;
										}
									}
								};
								if (globalModeCheckbox) {
									globalModeCheckbox.addEventListener('change', updateEntityNameDisabled);
								}
								if (globalEntityCapEnabled) {
									globalEntityCapEnabled.addEventListener('change', updateEntityNameDisabled);
								}
								if (globalEntityCapIgnoreEnabled) {
									globalEntityCapIgnoreEnabled.addEventListener('change', updateEntityNameDisabled);
								}
								updateEntityNameDisabled();
							})();
							</script>
							<p>
							...
							<p>
								<label><input type="checkbox" name="pmm_prune_preview_only" value="1" checked> <?php esc_html_e('Preview prune report only (no changes saved)', 'perchance-memory-manager'); ?></label>
								<span class="description" style="margin-left:6px;"><?php esc_html_e('Recommended: Always preview first to see what will be removed. Uncheck to apply changes.', 'perchance-memory-manager'); ?></span>
							</p>
						...
						<?php if ($prune_preview && !empty($prune_preview_data['stats']) && is_array($prune_preview_data['stats'])) : ?>
							...
							$preview_global_mode = !empty($prune_preview_data['global_mode']);
							$preview_target = $preview_global_mode
								? __('(all sections and entity buckets)', 'perchance-memory-manager')
								: ($preview_entity !== '' ? $preview_entity : __('(section-level entries)', 'perchance-memory-manager'));
							$preview_time_budget_exceeded = !empty($prune_preview_data['time_budget_exceeded']);
							?>
							<details style="margin-top:12px;" open>
								...
								<?php if ($preview_time_budget_exceeded) : ?>
									<div class="notice notice-warning" style="display:block; padding:12px; margin:8px 0; border-left:4px solid #ffb900;">
										<p><strong><?php esc_html_e('Note:', 'perchance-memory-manager'); ?></strong> <?php esc_html_e('Analysis was incomplete due to processing time limit. Results below show partial analysis of your entries. To complete the analysis, consider:', 'perchance-memory-manager'); ?></p>
										<ul style="margin:8px 0 0 20px;">
											<li><?php esc_html_e('Running prune in smaller chunks (single entity at a time)', 'perchance-memory-manager'); ?></li>
											<li><?php esc_html_e('Disabling global duplicate detection if enabled', 'perchance-memory-manager'); ?></li>
											<li><?php esc_html_e('Reducing the maximum keep value', 'perchance-memory-manager'); ?></li>
										</ul>
									</div>
								<?php endif; ?>
								<ul class="pmm-stats">
									<li><strong><?php esc_html_e('Before:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($preview_stats['original'] ?? 0))); ?></li>
									<li><strong><?php esc_html_e('After (estimated):', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) (((int) ($preview_stats['original'] ?? 0)) - ((int) ($preview_stats['exact_duplicates'] ?? 0)) - ((int) ($preview_stats['near_duplicates'] ?? 0)) - ((int) ($preview_stats['global_duplicates'] ?? 0)) - ((int) ($preview_stats['stale_removed'] ?? 0)) - ((int) ($preview_stats['unreferenced_removed'] ?? 0)) - ((int) ($preview_stats['entity_name_mismatch_removed'] ?? 0)) - ((int) ($preview_stats['entity_name_cap_removed'] ?? 0)) - ((int) ($preview_stats['trimmed'] ?? 0)))); ?></li>
									...
									<li><strong><?php esc_html_e('Global duplicates:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($preview_stats['global_duplicates'] ?? 0))); ?></li>
									...
									<li><strong><?php esc_html_e('Global entity-name cap removed:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($preview_stats['entity_name_cap_removed'] ?? 0))); ?></li>
								...
								$preview_groups = [
									...
									'global_duplicates' => __('Sample global duplicates to remove', 'perchance-memory-manager'),
									...
									'entity_name_cap_removed' => __('Sample entries removed by global entity-name cap', 'perchance-memory-manager'),
									...
									<details style="margin-top:8px;">
										...
										<ul style="margin:8px 0 0 16px;">
												<?php foreach ($items as $item) :
													$item_entry = is_array($item) ? (string) ($item['entry'] ?? '') : (string) $item;
													$item_matched = is_array($item) ? (string) ($item['matched'] ?? '') : '';
												?>
													<li>
														<?php echo esc_html($item_entry); ?>
														<?php if ($item_matched !== '') : ?>
															<span style="display:block;margin-top:2px;color:#666;font-size:0.9em;"><?php esc_html_e('↳ Matched:', 'perchance-memory-manager'); ?> <?php echo esc_html($item_matched); ?></span>
														<?php endif; ?>
													</li>
												<?php endforeach; ?>
										</ul>
								...
								?>
									<details style="margin-top:8px;" open>
										...
										<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="pmm-prune-preview-review-form" style="margin-top:8px;">
											...
											<input type="hidden" name="pmm_prune_global_mode" value="<?php echo $preview_global_mode ? '1' : '0'; ?>">
											...
											<table class="widefat striped" style="table-layout:fixed;width:100%;margin-top:6px;">
												<thead>
													<tr>
														<th style="width:12%;"><?php esc_html_e('Reason', 'perchance-memory-manager'); ?></th>
														<th style="width:10%;"><?php esc_html_e('Section', 'perchance-memory-manager'); ?></th>
														<th style="width:12%;"><?php esc_html_e('Entity', 'perchance-memory-manager'); ?></th>
														<th style="width:20%;"><?php esc_html_e('Original Entry', 'perchance-memory-manager'); ?></th>
														<th style="width:34%;"><?php esc_html_e('Edited Entry', 'perchance-memory-manager'); ?></th>
														<th style="width:8%;"><?php esc_html_e('Action', 'perchance-memory-manager'); ?></th>
														<th style="width:4%;"><?php esc_html_e('Critical', 'perchance-memory-manager'); ?></th>
													</tr>
												</thead>
												<tbody>
													<?php foreach ($review_candidates as $row_index => $row) : ?>
														...
														$source_section = isset($row['source_section']) ? (string) $row['source_section'] : $preview_section;
														$source_entity = isset($row['source_entity']) ? (string) $row['source_entity'] : $preview_entity;
														$source_bucket = isset($row['source_bucket']) ? (string) $row['source_bucket'] : ($source_entity !== '' ? $source_entity : '__entries__');
														$reason_label = isset($row['reason_label']) ? (string) $row['reason_label'] : __('Prune candidate', 'perchance-memory-manager');
														$matched_entry = isset($row['matched_entry']) ? (string) $row['matched_entry'] : '';
														$default_action = isset($row['default_action']) ? (string) $row['default_action'] : 'keep';
														if ($source_entry === '') {
															continue;
														}
														?>
														<tr class="pmm-prune-review-row" data-source-index="<?php echo esc_attr((string) $source_index); ?>" data-source-entry="<?php echo esc_attr($source_entry); ?>" data-source-section="<?php echo esc_attr($source_section); ?>" data-source-entity="<?php echo esc_attr($source_entity); ?>" data-source-bucket="<?php echo esc_attr($source_bucket); ?>">
															<td style="white-space:pre-wrap;"><?php echo esc_html($reason_label); ?></td>
															<td style="white-space:pre-wrap;"><?php echo esc_html($source_section); ?></td>
															<td style="white-space:pre-wrap;"><?php echo esc_html($source_entity !== '' ? $source_entity : __('(section-level)', 'perchance-memory-manager')); ?></td>
															<td style="white-space:pre-wrap;">
																<?php echo esc_html($source_entry); ?>
																<?php if ($matched_entry !== '') : ?>
																	<span style="display:block;margin-top:4px;padding:4px 6px;background:#f0f0f0;border-left:3px solid #aaa;font-size:0.9em;color:#444;"><?php esc_html_e('↳ Matched:', 'perchance-memory-manager'); ?> <?php echo esc_html($matched_entry); ?></span>
																<?php endif; ?>
															</td>
															...
															<td>
																<select class="pmm-prune-review-action" style="width:100%;">
																	<option value="keep" <?php selected($default_action, 'keep'); ?>><?php esc_html_e('Keep', 'perchance-memory-manager'); ?></option>
																	<option value="remove" <?php selected($default_action, 'remove'); ?>><?php esc_html_e('Remove', 'perchance-memory-manager'); ?></option>
										...
										(function () {
											...
											form.addEventListener('submit', function () {
												...
												form.querySelectorAll('tr.pmm-prune-review-row').forEach(function (row) {
													...
													var sourceBucket = row.getAttribute('data-source-bucket') || '';
			...
			<div class="pmm-card">
				<details class="pmm-collapsible-section">
					<summary><strong><?php esc_html_e('Global Entity Names', 'perchance-memory-manager'); ?></strong></summary>
					<p><?php esc_html_e('Maintain one global list of entity names (one per line). When this list is non-empty, prune/unreferenced matching uses this list only.', 'perchance-memory-manager'); ?></p>
					<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:8px 0 12px 0;">
						<?php wp_nonce_field('pmm_save_global_entity_names'); ?>
						<input type="hidden" name="action" value="pmm_save_global_entity_names">
						<textarea name="pmm_global_entity_names_text" rows="8" class="large-text code" placeholder="Echo-7&#10;Black Max&#10;Eva Thorne"><?php echo esc_textarea($global_entity_names_text); ?></textarea>
						<p class="description" style="margin-top:8px;"><?php esc_html_e('Tips: one name per line, duplicates are removed automatically, matching is case-insensitive.', 'perchance-memory-manager'); ?></p>
						<?php submit_button(__('Save Global Entity Names', 'perchance-memory-manager'), 'secondary', 'submit', false); ?>
					</form>

					<?php if (!empty($global_entity_names)) : ?>
						<p class="description"><strong><?php echo esc_html(sprintf(__('Total global entity names: %d', 'perchance-memory-manager'), count($global_entity_names))); ?></strong></p>
						<?php if (!empty($global_entity_match_counts)) : ?>
							<p class="description"><?php esc_html_e('Current output match counts (top 25):', 'perchance-memory-manager'); ?></p>
							<div style="overflow:auto;max-height:300px;border:1px solid #dcdcde;background:#fff;">
								<table class="widefat striped">
									<thead><tr><th><?php esc_html_e('Entity Name', 'perchance-memory-manager'); ?></th><th><?php esc_html_e('Entries Containing Name', 'perchance-memory-manager'); ?></th></tr></thead>
									<tbody>
										<?php foreach (array_slice($global_entity_match_counts, 0, 25, true) as $name => $count) : ?>
											<tr><td><?php echo esc_html((string) $name); ?></td><td><?php echo esc_html((string) ((int) $count)); ?></td></tr>
										<?php endforeach; ?>
									</tbody>
								</table>
							</div>
						<?php elseif (!empty($data['cleaned_data']) && is_array($data['cleaned_data'])) : ?>
							<p class="description"><?php esc_html_e('No entries matched your current global entity list in the loaded output.', 'perchance-memory-manager'); ?></p>
						<?php endif; ?>
					<?php endif; ?>
				</details>
			</div>
			...
			<div class="pmm-card">
				<details class="pmm-collapsible-section">
					...
					<p style="margin:8px 0;">
						...
						<span id="pmm_copy_perchance_export_status" class="description" style="margin-left:8px;"></span>
					</p>
					<?php if (!empty($db_perchance_export_entries)) : ?>
						<p class="description"><strong><?php echo esc_html(sprintf(__('Export entries: %d', 'perchance-memory-manager'), count($db_perchance_export_entries))); ?></strong></p>
						<textarea readonly id="pmm_perchance_export_text" rows="14" class="large-text code" style="width:100%;max-width:100%;box-sizing:border-box;white-space:pre;"><?php echo esc_textarea($db_perchance_export_text); ?></textarea>
					<?php else : ?>
						<p class="description"><?php esc_html_e('No active entries available. Rebuild/import entries first, or ensure rows are not marked as pruned.', 'perchance-memory-manager'); ?></p>
						<textarea readonly id="pmm_perchance_export_text" rows="6" class="large-text code" style="width:100%;max-width:100%;box-sizing:border-box;white-space:pre;"></textarea>
					<?php endif; ?>
					<script>
						document.addEventListener('DOMContentLoaded', function () {
							var copyBtn = document.getElementById('pmm_copy_perchance_export');
							var output = document.getElementById('pmm_perchance_export_text');
							var status = document.getElementById('pmm_copy_perchance_export_status');
							if (!copyBtn || !output || !status) {
								return;
							}
							copyBtn.addEventListener('click', function () {
								var text = output.value || '';
								if (!text) {
									status.textContent = <?php echo wp_json_encode(__('Nothing to copy yet.', 'perchance-memory-manager')); ?>;
									return;
								}
								if (navigator.clipboard && navigator.clipboard.writeText) {
									navigator.clipboard.writeText(text).then(function () {
										status.textContent = <?php echo wp_json_encode(__('Copied.', 'perchance-memory-manager')); ?>;
									}, function () {
										output.focus();
										output.select();
										status.textContent = <?php echo wp_json_encode(__('Clipboard blocked. Press Ctrl+C after selection.', 'perchance-memory-manager')); ?>;
									});
									return;
								}
								output.focus();
								output.select();
								status.textContent = <?php echo wp_json_encode(__('Select and press Ctrl+C.', 'perchance-memory-manager')); ?>;
							});
						});
					</script>
				</details>
			...
			<div class="pmm-card">
				<details class="pmm-collapsible-section">
					<summary><strong><?php esc_html_e('Database Workflow (Strict Entity Mode)', 'perchance-memory-manager'); ?></strong></summary>
					<p><?php esc_html_e('Safe-step DB mode: rebuild entries from latest output, keep Unknown as default when no list match, then manually re-tag Unknown to known entities (or add new entity + assign).', 'perchance-memory-manager'); ?></p>
					<ul>
						<li><strong><?php esc_html_e('Entries in DB:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($db_stats['total_entries'] ?? 0))); ?></li>
						<li><strong><?php esc_html_e('Unknown entries:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($db_stats['unknown_entries'] ?? 0))); ?></li>
						<li><strong><?php esc_html_e('Known entries:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($db_stats['known_entries'] ?? 0))); ?></li>
						<li><strong><?php esc_html_e('Entity names in DB:', 'perchance-memory-manager'); ?></strong> <?php echo esc_html((string) ((int) ($db_stats['total_entities'] ?? 0))); ?></li>
					</ul>

					<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:8px 0;display:inline-block;">
						<?php wp_nonce_field('pmm_db_rebuild_from_latest_output'); ?>
						<input type="hidden" name="action" value="pmm_db_rebuild_from_latest_output">
					...
					<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:8px 0 12px 8px;display:inline-block;vertical-align:top;">
						<?php wp_nonce_field('pmm_db_generate_dedupe_preview'); ?>
						<input type="hidden" name="action" value="pmm_db_generate_dedupe_preview">
						<label style="margin-right:6px;"><?php esc_html_e('Scope', 'perchance-memory-manager'); ?>
							<select name="pmm_db_dedupe_scope">
								<option value="all" <?php selected($db_dedupe_scope, 'all'); ?>><?php esc_html_e('All', 'perchance-memory-manager'); ?></option>
								<option value="unknown" <?php selected($db_dedupe_scope, 'unknown'); ?>><?php esc_html_e('Unknown only', 'perchance-memory-manager'); ?></option>
								<option value="known" <?php selected($db_dedupe_scope, 'known'); ?>><?php esc_html_e('Known only', 'perchance-memory-manager'); ?></option>
							</select>
						</label>
						<label style="margin-right:6px;"><?php esc_html_e('Near threshold', 'perchance-memory-manager'); ?>
							<input type="number" step="0.01" min="0.70" max="0.99" name="pmm_db_dedupe_threshold" value="<?php echo esc_attr((string) $db_dedupe_threshold); ?>" style="width:90px;">
						</label>
						<label style="margin-right:6px;"><?php esc_html_e('Max pairs', 'perchance-memory-manager'); ?>
							<input type="number" min="25" max="1000" name="pmm_db_dedupe_limit" value="<?php echo esc_attr((string) $db_dedupe_limit); ?>" style="width:90px;">
						</label>
						<?php submit_button(__('3) Generate DB Dedupe Preview', 'perchance-memory-manager'), 'secondary', 'submit', false); ?>
					</form>

					<p class="description" style="margin-top:0;"><?php esc_html_e('Dedupe Review table appears directly below after Step 3 runs.', 'perchance-memory-manager'); ?></p>

					<?php if (!empty($db_dedupe_candidates)) : ?>
						<h4 style="margin:10px 0 6px 0;"><?php esc_html_e('Dedupe Review Table', 'perchance-memory-manager'); ?></h4>
						<p class="description" style="margin-top:8px;"><?php esc_html_e('Review candidates below. Default is prune for exact duplicates and keep for near duplicates. Chronological sequence is preserved; pruning marks duplicate rows with status=pruned.', 'perchance-memory-manager'); ?></p>
						<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:8px 0 12px 0;">
							<?php wp_nonce_field('pmm_db_apply_dedupe_review'); ?>
							<input type="hidden" name="action" value="pmm_db_apply_dedupe_review">
							<input type="hidden" name="pmm_db_dedupe_rows" id="pmm_db_dedupe_rows" value="">
							<div style="overflow:auto;max-height:360px;border:1px solid #dcdcde;background:#fff;">
								<table class="widefat striped">
									<thead><tr><th><?php esc_html_e('Action', 'perchance-memory-manager'); ?></th><th><?php esc_html_e('Type', 'perchance-memory-manager'); ?></th><th><?php esc_html_e('Similarity', 'perchance-memory-manager'); ?></th><th><?php esc_html_e('Keep', 'perchance-memory-manager'); ?></th><th><?php esc_html_e('Duplicate', 'perchance-memory-manager'); ?></th></tr></thead>
									<tbody>
									<?php foreach ($db_dedupe_candidates as $pair) : ?>
										<?php
										$keep_id = (int) ($pair['keep_entry_id'] ?? 0);
										$dup_id = (int) ($pair['duplicate_entry_id'] ?? 0);
										$type = isset($pair['type']) ? (string) $pair['type'] : 'near';
										$similarity = isset($pair['similarity']) ? (float) $pair['similarity'] : 0.0;
										$default_action = isset($pair['default_action']) ? (string) $pair['default_action'] : 'keep';
										?>
										<tr>
											<td>
												<input type="hidden" class="pmm-db-dedupe-dup-id" value="<?php echo esc_attr((string) $dup_id); ?>">
												<input type="hidden" class="pmm-db-dedupe-keep-id" value="<?php echo esc_attr((string) $keep_id); ?>">
												<input type="hidden" class="pmm-db-dedupe-type" value="<?php echo esc_attr($type); ?>">
												<input type="hidden" class="pmm-db-dedupe-similarity" value="<?php echo esc_attr((string) $similarity); ?>">
												<select class="pmm-db-dedupe-action">
													<option value="keep" <?php selected($default_action, 'keep'); ?>><?php esc_html_e('Keep both', 'perchance-memory-manager'); ?></option>
													<option value="prune" <?php selected($default_action, 'prune'); ?>><?php esc_html_e('Prune duplicate', 'perchance-memory-manager'); ?></option>
												</select>
											</td>
											<td><?php echo esc_html($type); ?></td>
											<td><?php echo esc_html(number_format($similarity, 2)); ?></td>
											<td style="white-space:pre-wrap;"><strong>#<?php echo esc_html((string) $keep_id); ?></strong> (<?php echo esc_html((string) ((int) ($pair['keep_seq'] ?? 0))); ?>)<?php echo "\n" . esc_html((string) ($pair['keep_text'] ?? '')); ?></td>
											<td style="white-space:pre-wrap;"><strong>#<?php echo esc_html((string) $dup_id); ?></strong> (<?php echo esc_html((string) ((int) ($pair['duplicate_seq'] ?? 0))); ?>)<?php echo "\n" . esc_html((string) ($pair['duplicate_text'] ?? '')); ?></td>
										</tr>
									<?php endforeach; ?>
									</tbody>
								</table>
							</div>
							<?php submit_button(__('4) Apply Dedupe Review', 'perchance-memory-manager'), 'primary', 'submit', false, ['style' => 'margin-top:8px;']); ?>
						</form>
						<script>
							document.addEventListener('DOMContentLoaded', function () {
								var form = document.querySelector('form[action$="admin-post.php"] input[name="action"][value="pmm_db_apply_dedupe_review"]');
								if (!form) {
									return;
								}
								var dedupeForm = form.closest('form');
								if (!dedupeForm) {
									return;
								}
								dedupeForm.addEventListener('submit', function () {
									var rows = [];
									var actionEls = dedupeForm.querySelectorAll('.pmm-db-dedupe-action');
									var dupIdEls = dedupeForm.querySelectorAll('.pmm-db-dedupe-dup-id');
									var keepIdEls = dedupeForm.querySelectorAll('.pmm-db-dedupe-keep-id');
									var typeEls = dedupeForm.querySelectorAll('.pmm-db-dedupe-type');
									var simEls = dedupeForm.querySelectorAll('.pmm-db-dedupe-similarity');
									for (var i = 0; i < actionEls.length; i++) {
										var duplicateId = dupIdEls[i] ? parseInt(dupIdEls[i].value, 10) : 0;
										var keepId = keepIdEls[i] ? parseInt(keepIdEls[i].value, 10) : 0;
										if (!duplicateId) {
											continue;
										}
										rows.push({
											action: actionEls[i].value,
											keep_entry_id: keepId,
											duplicate_entry_id: duplicateId,
											type: typeEls[i] ? typeEls[i].value : 'near',
											similarity: simEls[i] ? parseFloat(simEls[i].value) : 0
										});
									}
									var payload = dedupeForm.querySelector('#pmm_db_dedupe_rows');
									if (payload) {
					...
					<?php else : ?>
						<p class="description" style="margin-top:8px;"><?php esc_html_e('No dedupe review rows are loaded yet. Click "3) Generate DB Dedupe Preview" to populate this table.', 'perchance-memory-manager'); ?></p>
					<?php endif; ?>

					<?php if (!empty($db_unknown_rows)) : ?>
							...
							<table class="widefat striped">
								...
								<?php foreach ($db_unknown_rows as $db_row) : ?>
									...
									<tr>
										...
										<td><?php echo esc_html((string) ($db_row['section_name'] ?? '')); ?></td>
										<td><div style="white-space:pre-wrap;max-height:120px;overflow:auto;"><?php echo esc_html((string) ($db_row['entry_text'] ?? '')); ?></div></td>
										<td>
											<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:0;">
												<?php wp_nonce_field('pmm_db_unknown_entry_save'); ?>
												<input type="hidden" name="action" value="pmm_db_unknown_entry_save">
												...
												<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
													...
													<button type="submit" class="button button-link-delete" style="margin:0;" onclick="document.getElementById('pmm_row_action_<?php echo esc_js((string) $db_entry_id); ?>').value='prune';return true;"><?php esc_html_e('Remove', 'perchance-memory-manager'); ?></button>
												</div>
											</form>
										</td>
									</tr>
			...
			<?php if (false && $has_last_output && !empty($data['entity_report']) && is_array($data['entity_report'])) : ?>
				...
				<div class="pmm-card">
					<details class="pmm-collapsible-section pmm-collapsible-root">
						...
						<div style="margin-top:10px;">
							<?php $this->render_entity_review($data['entity_report']['entities'] ?? [], $entity_review_queue, $entity_queue_filter, $data['cleaned_data'] ?? []); ?>
	...
	private function build_progress($job) {
		...
		if ($stage === 'prune_global') {
			$total = max(1, count((array) ($job['targets'] ?? [])));
			$done = min($total, (int) ($job['target_index'] ?? 0));
			$ratio = $done / $total;

			return [
				'percent' => (int) floor($ratio * 95),
				'label' => __('Global Prune', 'perchance-memory-manager'),
				'detail' => sprintf(__('Pruned %1$d of %2$d section/entity buckets.', 'perchance-memory-manager'), $done, $total),
			];
		}

		if ($stage === 'prune_global_finalize') {
			return [
				'percent' => 98,
				'label' => __('Finalizing Global Prune', 'perchance-memory-manager'),
				'detail' => __('Applying global cap and building final review queue.', 'perchance-memory-manager'),
			];
		}
	...
	private function render_entity_review($entity_groups, $queue = [], $queue_filter = 'pending', $cleaned_data = []) {
		...
		foreach ((array) $entity_groups as $section => $names) {
			foreach ((array) $names as $name) {
				...
				$rows[] = [
					...
					'name' => $name,
					'key' => $key,
					'entries_text' => $this->entity_entries_text((array) $cleaned_data, $section, $name),
				];
			}
		}
		...
		echo '<p class="description">' . esc_html__('Keep leaves the entity untouched. You can also edit entry lines directly per entity. Keep and hide excludes it from future entity review prompts. Remove deletes the entity and entries that mention it when you reprocess.', 'perchance-memory-manager') . '</p>';
		...
		echo '<table class="widefat striped" style="margin-top:8px;min-width:980px;">';
		echo '<thead><tr>';
		echo '<th>' . esc_html__('Section', 'perchance-memory-manager') . '</th>';
		echo '<th>' . esc_html__('Entity', 'perchance-memory-manager') . '</th>';
		echo '<th>' . esc_html__('Action', 'perchance-memory-manager') . '</th>';
		echo '<th>' . esc_html__('Touched', 'perchance-memory-manager') . '</th>';
		echo '<th>' . esc_html__('Entries (editable)', 'perchance-memory-manager') . '</th>';
		echo '</tr></thead><tbody>';
		...
		foreach ($rows_page as $row) {
			...
			echo '</td>';
			echo '<td>' . esc_html($is_reviewed ? __('Reviewed', 'perchance-memory-manager') : __('Pending', 'perchance-memory-manager')) . '</td>';
			echo '<td style="min-width:420px;">';
			echo '<textarea name="pmm_entities[' . esc_attr($id) . '][entries_text]" rows="5" class="large-text code" style="width:100%;max-width:100%;box-sizing:border-box;">' . esc_textarea((string) ($row['entries_text'] ?? '')) . '</textarea>';
			echo '</td>';
			echo '</tr>';
		}
	...
	private function get_global_entity_names_option() {
		$stored = get_option('pmm_global_entity_names', []);
		return $this->normalize_global_entity_names($stored);
	}

	private function normalize_global_entity_names($value) {
	...
	private function count_global_entity_matches($cleaned_data, $entity_names) {
		$counts = [];
		$threshold = 0.60;
		$entries = [];

		foreach ((array) $cleaned_data as $section => $content) {
			if ((string) $section === '__chronological__') {
				continue;
			}
			if (!is_array($content)) {
				continue;
		...
		foreach ((array) $entity_names as $name) {
			...
			foreach ($entries as $entry) {
				if (stripos($entry, $name) !== false || PMM_Utils::contains_name_score($entry, $name) >= $threshold) {
					$hit_count++;
				}
			}
			if ($hit_count > 0) {
